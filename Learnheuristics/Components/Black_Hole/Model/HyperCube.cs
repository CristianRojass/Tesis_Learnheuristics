...
namespace Learnheuristics.Components.Black_Hole.Model {

    //Representa una zona del hiperespacio (n-dimensional) análoga a un cuadrado o cubo.
    //Además, contiene hiperpuntos (n-dimensional) dentro, los que luego se convertirán en estrellas del black hole.
    public class HyperCube {

        //Puntos de esquina que definen el hipercubo.
        public List<Vector> corner_hyper_points;

        //Puntos contenidos en el hipercubo: Serán las estrellas del Black Hole.
        ...
        #region Creación de puntos esquina/internos
        //Crea la envoltura hipercúbica alrededor del punto central.
        private List<Vector> GetWrapper(float scale) {
            ...
            static List<Vector> CreateCenterWrapper(int dimensional_size, float scale) {

                //Obtiene la secuencia de componentes que definen el n-ésimo punto de esquina según su representación en binario.
                static string GetFullyBinaryRepresentation(int value, int dimensional_size) {
                    var binary_string_representation = Convert.ToString(value, 2);
                    while (binary_string_representation.Length != dimensional_size)
                ...
                var amount_of_points = Math.Pow(2, dimensional_size);
                for (int point = 0; point < amount_of_points; point++) {
                    var hyper_point = GetFullyBinaryRepresentation(point, dimensional_size).Select(coordinate_representation => {
                ...
                Lo anterior sugiere figura simetrica. Además, la secuencia generada por la representación binaria para crear los vectores sugiere red cúbica. */
                ...
                if (!success)
                    throw new ApplicationException($"El hipercubo {dimensional_size} dimensional que ha sido creado no cumple con las validaciones.");
                #endregion

                return centered_hypercube_corner_points;
            }

            var hypercube = CreateCenterWrapper(middle_hyper_point.DimensionalSize, scale).Select(corner_hyper_point => Vector.Add(corner_hyper_point, middle_hyper_point)).ToList();
            return hypercube;
        }
        ...
        private List<Vector> GetInnerPointsNetwork(int depth) {
            ...
            var inner_points_network = GetHypercubicNetwork(middle_hyper_point, Convert.ToSingle(Math.Round(distance_inner_points, 6)), depth + 1);
        ...
        #region Creación de redes hipercúbicas de puntos/hipercubos
        //Crea red hipercubica de puntos en el hiperespacio centrados en el <middle_hyper_point>.
        private static List<Vector> GetHypercubicNetwork(Vector middle_hyper_point, float scale, int depth) {

            //Crea red hipercubica de puntos en el hiperespacio.
            static List<Vector> CreateCenterHypercubicNetwork(int dimensional_size, float scale, int depth) {
            ...
            //Centra la red hipercúbica de puntos en el punto central.
        ...
        public static List<HyperCube> CreateHyperCubeNetwork(Vector middle_hyper_point, float length, int factor, int depth, int inner_depth) {
        ...
        #region Métodos sin uso actualmente.
